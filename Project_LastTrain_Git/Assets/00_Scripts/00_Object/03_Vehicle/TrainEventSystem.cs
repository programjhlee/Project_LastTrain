...
public class TrainEventSystem : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        eventID = new int[_eventDataList.Count];
        ...
        for (int i = 0; i < _trainEventData.Count; i++)
        {
            ...
            Transform eventPool = new GameObject($"{_eventDataDics[eventID[i]].EventName}").GetComponent<Transform>();
    ...
    public Event SpawnEventAt(float normalizeXPos, int? eventIdx = null)
    {
        ...
        evt = _inactiveEventPools[eventID[selectIdx]].Pop();
        ...
        evt.Enter(_eventDataDics[eventID[selectIdx]], xPos, yPos);
        BindTrainEvent(evt);
